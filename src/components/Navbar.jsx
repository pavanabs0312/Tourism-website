...
import { 
  ...
  MapPin, User, LogOut, Radio, Layers, Bell, Building, Sparkles, Route 
} from 'lucide-react';

export const Navbar = ({ onOpenDemoModal }) => {
  const { currentUser, activeMode, logout } = useAuth();
  const { sosActive } = useTourist();
  const { kpiStats } = useAuthority();
  const location = useLocation();
  const navigate = useNavigate();

  const isAuthority = activeMode === 'AUTHORITY';

  // Tourist Navigation Links (strictly tourist journey only - NO authority links & NO scenarios)
  const touristNavLinks = [
    { path: '/tourist', label: 'Dashboard', icon: Compass },
    { path: '/destination-planner', label: 'Plan Trip', icon: Sparkles },
    { path: '/trip-planner', label: 'Smart Planner', icon: Route },
    { path: '/navigation', label: 'Navigation', icon: Navigation },
    { path: '/safety', label: 'Safety Engine', icon: Shield },
  ...
  const authorityNavLinks = [
    ...
    { path: '/authority/tourists', label: 'Live Tourists', icon: User, badge: kpiStats.totalTourists },
  ...
  const currentLinks = isAuthority ? authorityNavLinks : touristNavLinks;
  ...
  const displayName = isAuthority ? (currentUser?.name || 'Officer K. Naik') : (currentUser?.name || 'pav');
  ...
  return (
    <header style={{ backgroundColor: '#0a1128', borderBottom: '1px solid rgba(255,255,255,0.1)', position: 'sticky', top: 0, zIndex: 1000 }}>
      ...
      <div className="container-custom" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '68px', padding: '0 1rem' }}>
        {/* Brand & Logo */}
        <Link to={isAuthority ? '/authority' : '/tourist'} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', textDecoration: 'none' }}>
        ...
        {/* Center Nav Links (Strictly Role-Isolated) */}
        <nav style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
          {currentLinks.map((link) => {
            const Icon = link.icon;
            const isActive = location.pathname === link.path || 
              (link.path === '/destination-planner' && location.pathname === '/trip-planner');
        ...
        {/* Right Action Controls: Role Profile & Logout */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
          {/* SIH Demo Scenarios Trigger — Rendered ONLY in Authority Mode */}
          {isAuthority && (
            <button
